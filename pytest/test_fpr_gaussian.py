...
def test_init1(fpr1):
    ...
    assert np.allclose([-0.14, -0.07, 0., 0.07, 0.14], fpr1.t1)
    assert np.allclose([-0.14, -0.07, 0., 0.07, 0.14], fpr1.t2)
    assert np.allclose([(0.19568007574725543, -2.7908622928847295),
    ...
    assert np.allclose([(19.804319924252745, -2.7908622928847295),
...
def test_init2(fpr2):
    """Tests the initialization of the class."""
    assert np.allclose([-0.14, 0., 0.14], fpr2.t1)
    assert np.allclose([-0.35, -0.25, -0.15, -0.05, 0.05, 0.15, 0.25, 0.35], fpr2.t2)
    assert np.allclose([(0.09784003787362772, -1.3954311464423648),
                        (0.0, 0.0),
                        (0.09784003787362772, 1.3954311464423648)],
                       fpr2.pos1
                       )
    assert np.allclose([(28.181181385421368, -10.286934223663541),
