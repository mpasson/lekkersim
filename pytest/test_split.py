...
def test_split_simple():
    ...
    with sv.Solver() as S:
        g1 = []
        g1.append(sv.Waveguide(1.0).put())
        g1.append(sv.Waveguide(1.0).put("a0", g1[-1].pin["b0"]))
        g1.append(sv.Waveguide(1.0).put("a0", g1[-1].pin["b0"]))
        sv.Pin("a0").put(g1[0].pin["a0"])
        sv.Pin("b0").put(g1[-1].pin["b0"])

        g2 = []
        g2.append(sv.Waveguide(1.0).put())
        g2.append(sv.Waveguide(1.0).put("a0", g2[-1].pin["b0"]))
        sv.Pin("a1").put(g2[0].pin["a0"])
        sv.Pin("b2").put(g2[-1].pin["b0"])

        g3 = []
        g3.append(sv.Waveguide(1.0).put())
        sv.Pin("a1").put(g3[0].pin["a0"])
        sv.Pin("b2").put(g3[-1].pin["b0"])
...
def test_split_parametric_mapped():
    with sv.Solver() as S:
        ...
        ps1.raise_pins()
        ps2.raise_pins(["a0", "b0"], ["a1", "b1"])
        sv.add_param("PS1", lambda V1=0.0: V1 ** 2.0)
        sv.add_param("PS2", lambda V2=0.0: V2 ** 2.0)
